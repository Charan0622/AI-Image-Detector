...
def extract_freq_features(image_pil: Image.Image) -> np.ndarray:
    ...
    Returns a 13-dimensional feature vector of spectral statistics.
    ...
    low = dct_log[:q1, :q1].mean()
    mid_low = dct_log[q1:q2, q1:q2].mean()
    mid_high = dct_log[q2:q3, q2:q3].mean()
    high = dct_log[q3:, q3:].mean()
    ...
    valid = radial[1:max_r] > 0
    if valid.sum() > 10:
        lf = np.log(np.arange(1, max_r))[valid]
        lp = np.log(radial[1:max_r])[valid]
        A = np.vstack([lf, np.ones(len(lf))]).T
        alpha, intercept = np.linalg.lstsq(A, lp, rcond=None)[0]
        predicted = alpha * lf + intercept
        residual = np.sqrt(np.mean((lp - predicted) ** 2))
    else:
        alpha, residual = -1.0, 0.0

    # Spectral entropy
    dct_norm = dct_abs.flatten()
    dct_norm = dct_norm / (dct_norm.sum() + 1e-10)
    entropy = -np.sum(dct_norm * np.log(dct_norm + 1e-15))

    # Grid artifact score
    fft_ac = np.abs(np.fft.ifft2(fft_mag ** 2))
    fft_ac = fft_ac / (fft_ac[0, 0] + 1e-8)
    center = fft_ac[1 : h // 8, 1 : w // 8]
    grid_score = center.max() if center.size > 0 else 0.0

    # Noise stats
    noise_map = laplace(gray)
    noise_std = noise_map.std()
    noise_kurtosis = float(
        np.mean((noise_map - noise_map.mean()) ** 4) / (noise_map.std() ** 4 + 1e-8)
    )
    ...
    for c in range(3):
        ...
        ch_abs = np.abs(ch_dct).flatten()
        ch_n = ch_abs / (ch_abs.sum() + 1e-10)
        ch_ents.append(-np.sum(ch_n * np.log(ch_n + 1e-15)))
    ch_var = np.var(ch_ents)
